  ...
  validation {
    condition = alltrue([
      ...
      (
        ...
        can(regex("^arn:[a-z0-9-]+:organizations::[0-9]{12}:ou/o-[a-z0-9]{10,32}/ou-[a-z0-9-]{4,32}$", principal))
      )
    ])
    error_message = "principals requires stable lowercase keys and a 12-digit account ID, Organization ARN, or OU ARN."
  }
}
